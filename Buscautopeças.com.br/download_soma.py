...
url = 'https://www.somapecas.com.br/biblioteca-digital-catalogos'
output_dir = 'Buscautopeças.com.br/catalogos_soma'
...
try:
    ...
    # Busca por href que termine em .pdf
    links = re.findall(r'href=[\'\"]([^\'\"]+\.pdf)[\'\"]', html, re.IGNORECASE)
    
    # Remove duplicatas
    full_links = list(set(links))
    
    # Tenta lidar com links relativos (caso não comecem com http)
    fixed_links = []
    for link in full_links:
        if link.startswith('http'):
            ...
            if not link.startswith('/'):
                link = '/' + link
            fixed_links.append('https://www.somapecas.com.br' + link)
            
    if not fixed_links:
        print("Nenhum PDF direto encontrado. Listando todos os links da página para análise:")
        ...
        for i, pdf_url in enumerate(fixed_links[:3]):
            ...
            # Algumas URLs podem ter interrogações no final, limpa o nome do arquivo
            ...
            try:
                ...
                with urllib.request.urlopen(pdf_req, timeout=30, context=ctx) as response, open(filepath, 'wb') as out_file:
