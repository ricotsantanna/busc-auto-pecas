...
url = 'https://www.montanna.com.br/catalogos/'
output_dir = 'Buscautopeças.com.br/catalogos_montanna'
...
try:
    ...
    for link in full_links:
        if link.startswith('http'):
            ...
            if not link.startswith('/'):
                link = '/' + link
            fixed_links.append('https://www.montanna.com.br' + link)
    ...
    if not fixed_links:
        ...
        for i, pdf_url in enumerate(fixed_links[:3]):
            ...
            if len(filename) < 5: filename = f"montanna_cat_{i}.pdf"
            filepath = os.path.join(output_dir, filename)
            
            print(f"Baixando ({i+1}/3): {filename}...")
            try:
                # O nome do arquivo pode ter espaços, encodando apenas o caminho
