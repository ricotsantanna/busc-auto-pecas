...
export default function LojistaEstoque() {
  ...
  // Spreadsheet Bulk Import States
  ...
  // Download Model CSV Template
  const handleDownloadModelCsv = () => {
    const csvHeader = "Descricao_Peca,Fabricante_Marca,Codigo_Fabricante,Preco,Quantidade,Montadora,Modelo_Veiculo,Anos_Compativeis\n";
    const exampleRows = [
      "Pastilha de Freio Dianteira,Cobreq,N-225,120.00,5,VW,Gol,2015-2020",
      "Disco de Freio Ventilação Dianteira,Fremax,BD-1234,185.50,10,Chevrolet,Onix,2017-2022",
      "Amortecedor Dianteiro Gas,Monroe,SP-001,340.00,4,Fiat,Palio,2010-2016",
    ].join("\n");

    const blob = new Blob(["\uFEFF" + csvHeader + exampleRows], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", "modelo_importacao_estoque_buscautopecas.csv");
  ...
  const handleProcessImportFile = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (rows.length === 0) {
        alert("A planilha selecionada está vazia ou não contém colunas válidas.");
        return;
      }

      const res = await fetch("/api/lojista/inventory/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      console.error(err);
      alert("Erro ao ler o arquivo. Certifique-se de que é um CSV válido.");
    } finally {
      setImporting(false);
    }
  };

  // Confirm Import into D1
  const handleConfirmImport = async () => {
    if (stagingItems.length === 0) return;

    setConfirmingImport(true);
    try {
      const res = await fetch("/api/lojista/inventory/import/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        // Recarrega a lista de ofertas do estoque
        const offRes = await fetch("/api/lojista/offers");
        if (offRes.ok) {
          const offData = await offRes.json();
          setOffers(offData.offers || []);
        }
      } else {
        alert(data.error || "Erro ao confirmar importação no D1.");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            ...
            className="inline-flex items-center px-3.5 py-2 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-50 border border-slate-300 rounded-lg shadow-2xs transition-colors cursor-pointer"
            title="Baixar planilha CSV com exemplo de cabeçalhos e colunas"
          >
          ...
          <button
            ...
            className="inline-flex items-center px-4 py-2 text-xs font-semibold text-white bg-orange-600 hover:bg-orange-700 rounded-lg shadow-sm transition-colors cursor-pointer"
          >
            <FileSpreadsheet className="w-4 h-4 mr-1.5" />
            Carga em Lote (CSV/XLSX)
          </button>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-100">
            <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-slate-900">Importação em Lote via Planilha</h3>
              </div>
              <button
                onClick={() => {
                  setIsImportModalOpen(false);
            ...
            <form onSubmit={handleProcessImportFile} className="p-6 space-y-5">
              <div className="bg-orange-50/80 border border-orange-200 rounded-xl p-4 text-xs text-orange-950 space-y-2">
                <p className="font-semibold flex items-center gap-1.5 text-orange-900">
                  <Sparkles className="w-4 h-4 text-orange-600 shrink-0" />
                  Triagem Inteligente via IA Automotiva
                </p>
                <p>
                  Sua planilha será analisada pelo nosso assistente de IA. Itens reconhecidos serão vinculados ao <strong>Catálogo Canônico</strong>. Peças inéditas serão salvas na <strong>Fila de Triagem</strong> e enviadas para homologação do Administrador.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-slate-300 hover:border-orange-500 rounded-xl p-6 text-center bg-slate-50/50 transition-colors">
                  ...
                  <input
                    ...
                    className="block w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-orange-100 file:text-orange-800 hover:file:bg-orange-200 cursor-pointer"
                  />
                  {importFile && (
                    <p className="mt-2 text-xs font-semibold text-emerald-600 flex items-center justify-center gap-1">
                      <CheckCircle2 className="w-4 h-4" /> {importFile.name} ({(importFile.size / 1024).toFixed(1)} KB)
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                ...
                >
                  <Download className="w-3.5 h-3.5" /> Baixar Exemplo CSV
                </button>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-5 py-2 text-xs font-semibold text-white bg-orange-600 hover:bg-orange-700 rounded-lg shadow-sm transition-colors disabled:opacity-50 flex items-center gap-2"
                  >
      ...
      {/* Staging Table Modal (Pré-visualização do De-Para) */}
      {isStagingModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full overflow-hidden border border-slate-100 max-h-[90vh] flex flex-col">
            <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-lg font-bold text-slate-900">Pré-visualização e Conferência (De-Para)</h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  Verifique como cada item da sua planilha foi associado antes de salvar no estoque.
                </p>
              </div>
              <button
            ...
            {/* Barra de Estatísticas da Triagem */}
            <div className="bg-slate-900 text-white px-6 py-3 shrink-0 flex items-center justify-between text-xs font-medium">
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1.5 text-emerald-400 font-semibold">
                  <CheckCircle2 className="w-4 h-4" />
                  {stagingItems.filter((i) => i.status === "MATCHED").length} Reconhecidos no Catálogo Canônico
                </span>
                <span className="flex items-center gap-1.5 text-amber-400 font-semibold">
                  <Inbox className="w-4 h-4" />
                  {stagingItems.filter((i) => i.status === "NEW_INBOX").length} Peças Inéditas (Fila de Triagem IA)
                </span>
              </div>
              <span className="text-slate-400">Total: {stagingItems.length} linhas</span>
            </div>

            {/* Tabela de Staging com Scroll */}
              ...
              <table className="w-full text-left text-xs text-slate-600 border-collapse">
                <thead className="bg-slate-100 text-slate-700 font-semibold sticky top-0 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-3">Descrição da Planilha / De-Para</th>
                    <th className="px-4 py-3">Fabricante / Cód.</th>
                    <th className="px-4 py-3">Preço (R$)</th>
                    <th className="px-4 py-3">Qtd.</th>
                    <th className="px-4 py-3">Veículo Compatível</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                  {stagingItems.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} className={isMatched ? "bg-emerald-50/20" : "bg-amber-50/20"}>
                        ...
                        <td className="px-4 py-3">
                          <p className="font-semibold text-slate-900">{item.cleanPartName}</p>
                          <p className="text-[11px] text-slate-400 italic">Original: {item.rawPartName}</p>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className="font-medium text-slate-800">{item.manufacturer}</span>
                          <span className="text-[11px] text-slate-400 block">{item.manufacturerCode}</span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-slate-700">
                          {item.make || item.model ? (
                            <span>
                              {item.make} {item.model} <small className="text-slate-400">({item.years || "Todos"})</small>
                            </span>
                          ) : (
                            <span className="text-slate-400 italic">Multiveículos</span>
                          )}
                        </td>
                      </tr>
                    );
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 shrink-0 flex justify-between items-center">
              <p className="text-xs text-slate-500">
                Peças inéditas serão salvas e enviadas para homologação por e-mail para o administrador.
              </p>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="px-5 py-2 text-xs font-semibold text-white bg-orange-600 hover:bg-orange-700 rounded-lg shadow-sm transition-colors disabled:opacity-50 flex items-center gap-2 cursor-pointer"
                >
                  {confirmingImport && <Loader2 className="w-4 h-4 animate-spin" />}
                  Confirmar Importação no D1
                </button>
              </div>
            </div>
          </div>
